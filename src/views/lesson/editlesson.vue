...
export default {
  data () {
    ...
    const squery = new AV.Query('Lesson')
    var p = this
    squery.get(id).then((todo) => {
      // todo 就是 objectId 为 582570f38ac247004f39c24b 的 Todo 实例
      console.log('Lesson', todo)

      p.lessonForm.name = todo.get('name')
      p.lessonForm.lm =todo.get('lm')
      p.lessonForm.startTime=todo.get('startTime')
      p.lessonForm.endTime=todo.get('endTime')
    })
    var p = this
