  ...
  <div class="app-container">
    <el-form ref="queryForm" :model="queryParam" :inline="true">
      <el-form-item label="班级：">
        <el-select v-model="queryParam.group" filterable placeholder="请选择">
          <el-option
    ...
    <el-table v-loading="listLoading" :data="tableData" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column label="头像" width="100">
        <template slot-scope="scope"><img :src="scope.row.imageUrl" width="40" height="40" class="head_pic">
        </template>
      </el-table-column>
      <el-table-column prop="userLevel" label="班级" :formatter="levelFormatter" />
      ...
      <el-table-column width="220px" label="操作" align="center">
        <template slot-scope="{row}">
          <el-button size="mini" class="link-left" @click="handleAsyncClick(row)">同步</el-button>
          <router-link :to="{path:'/user/student/edit', query:{id:row.id}}" class="link-left">
            <el-button size="mini">编辑</el-button>
          </router-link>
          <!--
          <router-link :to="{path:'/log/user/list', query:{userId:row.id}}" group="link-left">
